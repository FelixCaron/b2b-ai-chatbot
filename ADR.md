...
Un journal, pas une spécification : chaque entrée décrit une décision au moment
où elle a été prise, et n'est pas mise à jour quand la réalité change ensuite.
...
- **Ordre** — les entrées ne sont pas triées. Le fichier descend de l'ADR 058
  ...
  est le seul repère.
- **Numéros** — 001 à 058. Les numéros 007 et 008 ont été attribués deux fois à
  des décisions différentes ; les deux entrées du 14 août 2026 portent
  désormais le suffixe `b` (007b, 008b) et le disent dans leur en-tête. Un
  renvoi à « ADR 007 » écrit ailleurs dans le dépôt désigne celui du 10 août.
- **Chemins** — les entrées antérieures au 12 septembre 2026 citent des chemins
  (`apps/admin/...`, `api/...` à la racine) qui n'existent plus depuis la
...
The widget's opening screen — greeting bubble, header title, status line, input
placeholder — was hardcoded English (`apps/widget/src/main.js`), regardless of what
language the scanned site itself is actually written in. Asked to have the first message
match the site's language, generated by AI rather than translated live per request, and
to keep the surrounding widget chrome in that same language too.

### Decision

New `generateWelcomeExperience()` in `api/lib/llm.js`, deliberately separate from
`generateWebsiteSummary()` (which stays English on purpose — it only ever feeds the
model's own context, never shown to a visitor). One LLM call detects the site's primary
language and returns the greeting plus the three chrome labels together, as one JSON
object — same call, same language, so the panel can't end up mismatched (an AI-generated
French greeting under an English placeholder, say).

Generated once per site, at the same point `generateWebsiteSummary()` already runs
(`api/crawler/scan.js`'s auto-summary step, and `api/crawler/summarize.js`'s manual
regenerate) — not per visitor, since a live LLM call on every widget load would add real
latency and cost to every page view for text that doesn't change between visits. Stored
on `site_summaries` (new columns: `language`, `welcome_message`, `ui_status_title`,
`ui_status_online`, `ui_input_placeholder`) — already the right table, since this is
AI-generated-once-per-site content, same category as the summary itself.

New `api/chat/init.js`: a read-only, public endpoint (same trust model as `api/chat/index.js`
— anonymous visitors are exactly who calls it) the widget fetches once on load, before
rendering, to swap the hardcoded English defaults for the real ones. Falls back to those
same English defaults — not an error — on any failure (no `site_summaries` row yet, the
generation itself failed silently at scan time, the fetch is slow or errors client-side):
this feature degrading is a widget that looks exactly like it did before it existed, never
a broken one.

### Consequences

- A visitor now gets greeted, and sees the panel, in the language the actual business
  operates in, not a translation-agnostic English default.
- Two extra LLM calls at scan time (once, cached) instead of one — acceptable; the
  alternative (folding this into the existing summary prompt) risked destabilizing a
  working prompt for unrelated content.
- Existing sites scanned before this migration have no `welcome_message` etc. yet — they
  fall back to the English defaults until their next scan/regenerate, same as any other
  AI-generated field added after the fact.
...
2. Ajout du paramètre d'URL `theme_color` dans `ClientOnboarding.jsx` pour qu'il soit propagé à `preview.html`.
...
## ADR 028 : Remplacement du parseur Markdown et du streaming SSE maison par des bibliothèques éprouvées
**Date :** *(non consignée — postérieure à l'ADR 027 du 16 août 2026)*
**Statut :** Accepté
...
## ADR 008b : Interdiction des Salutations Répétitives & Fallback de Résumé d'Entreprise
**Date :** 14 Août 2026
**Statut :** Accepté

*Numéro réattribué par erreur : un ADR 008 existait déjà (10 août 2026). Renommé 008b ici ; les renvois à « ADR 008 » ailleurs désignent celui du 10 août.*
...
## ADR 007b : Directives Anti-Hallucination Strictes, Contexte Temporel et Suppression des Modes de Test Mock
**Date :** 14 Août 2026
**Statut :** Accepté

*Numéro réattribué par erreur : un ADR 007 existait déjà (10 août 2026). Renommé 007b ici ; les renvois à « ADR 007 » ailleurs désignent celui du 10 août.*
...
### Décision
- Injection du chatbot natif dans l'application React `apps/admin/index.html` avec une clé virtuelle `B2B_ADMIN_COPILOT_KEY`.
...
- L'IA peut déclencher `navigate_to` pour changer la vue (dashboard, pricing, leads, about) dans l'application React.
### Conséquences
...
### Décision
1. Mise en place de \
equireAuthentication\ et `requireSiteOwnership` dans `api/lib/server-config.js`.
2. Les endpoints API qui modifient des données exigent désormais un header `Authorization: Bearer <token>` et vérifient que l'utilisateur est bien le propriétaire du `tenant_id` ET que le `site_id` appartient bien à ce tenant.
3. `ClientOnboarding.jsx` envoie maintenant les headers authentifiés lors de ses appels `fetch`.
...
### Décision
1. **API** : Regroupées par domaine métier (api/crawler/, api/billing/, api/chat/, api/cron/).
2. **Scripts** : Triés en sous-dossiers (scripts/tests/, scripts/ops/, scripts/adhoc/).
3. **Frontend** : Création du dossier features/dashboard/. Renommage de ClientOnboarding.jsx en Dashboard.jsx. Les APIs fetch ont été mises à jour.
...
### Décision
1. **Librairie Resend** ajoutée et configurée dans `api/lib/email.js` pour distribuer les courriels.
2. **Alertes Bug** branchées dans le bloc try/catch global de `api/chat/index.js`.
...
## ADR : Tickets support persistés (traçabilité des échecs Resend) et fusion des leads par conversation
**Date :** 9 Septembre 2026
**Statut :** Accepté

### Contexte
Deux signalements distincts, tous deux racines dans la même absence : rien de ce qui se passe côté LLM/outils n'est conservé indépendamment de l'email qu'il déclenche.
- **Demandes de support « perdues »** : `send_support_email` (`api/chat/index.js`) n'existait qu'en tant qu'email — succès ou échec, rien n'était jamais écrit en base. Un échec d'envoi ne laissait qu'un `console.error` dans un log Edge éphémère, invisible pour le tenant. Impossible de distinguer « le bot n'a pas appelé l'outil » de « l'outil a tourné mais l'email n'est jamais arrivé » — les deux se traduisaient, côté dashboard, par rigoureusement rien. (La cause racine réelle — un bug de code, pas une affaire de configuration — a été trouvée juste après ; voir l'ADR suivant, qui corrige aussi ce qui est écrit ici.)
- **Deux leads pour une seule conversation** : `extractLeadInfo` relit tout l'historique à chaque message tant que la capture de leads est active, et la déduplication ne matchait que sur le champ que *cette* extraction avait rempli (email si présent, sinon téléphone). Un visiteur donnant son email tôt puis son téléphone plus tard produit une extraction email-only suivie d'une extraction phone-only : la seconde ne trouve jamais la ligne créée par la première (elle n'a pas d'email sur lequel chercher), et une seconde ligne est insérée pour la même conversation.

### Décision
- **`support_tickets`** (nouvelle table, migration `20260909040000`, RLS `current_user_owns_tenant` comme `leads`) : chaque appel à `send_support_email` y écrit une ligne — `name`/`email`/`message` du visiteur, `delivered` (le vrai résultat de `sendSupportTicketEmail`), `session_id`, `tenant_id`, `site_id` — que l'envoi Resend réussisse ou non. `delivered = false` sur une ligne existante signifie très précisément « l'outil a tourné, l'email a échoué » ; l'absence totale de lignes signifierait que l'outil n'a jamais été appelé.
- **Visibilité dashboard** : `useAssistantHealth` compte les tickets `delivered = false` (tout l'historique, même logique que le compteur de questions sans réponse) ; `AssistantHealth` affiche une bannière d'alerte dédiée quand ce compteur est positif, menant à une nouvelle page `/support-requests` (`features/support/SupportTicketsPage.jsx` + `components/SupportTicketsTable.jsx`, même famille que `LeadsPage`/`LeadsTable`) qui liste chaque demande avec son statut de livraison et invite à relancer le visiteur manuellement par l'email conservé.
- **Fusion des leads par `session_id`** (colonne ajoutée aux `leads`, migration `20260909030000`) : à chaque extraction, `api/chat/index.js` cherche d'abord un lead existant pour `(tenant_id, session_id)` — la même conversation, quels que soient les champs capturés à chaque passage — avant de retomber sur l'ancien matching par email/téléphone (pour un visiteur qui revient dans une nouvelle session). Une ligne trouvée est mise à jour en fusionnant les champs (le plus récent non vide l'emporte, jamais d'effacement d'un champ déjà connu) plutôt que dupliquée ; une notification de lead n'est renvoyée que si la fusion a effectivement ajouté un nouveau canal de contact (email ou téléphone qui manquait).

### Conséquences
- Une demande de support qui échoue à l'envoi reste consultable et actionnable dans le dashboard au lieu d'être perdue.
- Une conversation ne peut plus produire deux leads : un email donné tôt et un téléphone donné plus tard finissent sur la même ligne.
- `npm test` (contrats, schémas, imports API) et `npm run build --workspace=@b2b-ai-chatbot/admin` passent après ces changements ; la suite E2E Playwright n'a pas pu être exécutée dans cet environnement (version de navigateur pré-installée différente de celle attendue par `@playwright/test`, sans rapport avec ces changements).

## ADR : L'échec d'envoi silencieux du SDK Resend, l'alerte côté plateforme, et la résolution des questions sans réponse
**Date :** 9 Septembre 2026
**Statut :** Accepté

### Contexte
L'ADR précédent supposait que les demandes de support non reçues venaient d'une configuration Resend (domaine non vérifié). C'était faux : **c'était un bug de code**, et il rendait le problème invisible par construction.

`resend.emails.send()` **ne lève jamais d'exception**. Une clé absente ou invalide (401), un domaine d'expédition non vérifié (403), un destinataire malformé (422), même une erreur réseau : tout revient dans une promesse *résolue* portant `{ data: null, error: {...} }`. Or les trois fonctions de `api/lib/email.js` faisaient `await resend.emails.send(...)` dans un `try/catch` et traitaient « ça n'a pas levé » comme « c'est envoyé ». Le `catch` était donc du code mort pour exactement les cas pour lesquels il avait été écrit. Conséquences en chaîne :
- `sendSupportTicketEmail` retournait `true` pour **tout** échec ; l'assistant annonçait au visiteur « votre message a bien été transmis, on vous répond bientôt » pour un email jamais parti, et le futur `support_tickets.delivered` aurait enregistré `true` sur ces échecs — le drapeau censé rendre le problème diagnosticable aurait menti lui aussi.
- Aucun `console.error` n'était émis : ni le tenant, ni nous, ne pouvions voir quoi que ce soit.
- `sendBugAlertEmail` souffrait du même défaut, donc **nos propres alertes de bug pouvaient échouer sans que personne ne le sache** — l'angle mort qui explique qu'un bug de cette taille ait pu durer.

Deuxième sujet, sans rapport de cause mais de même nature (une boucle qu'on ne peut pas fermer) : le compteur « questions sans réponse » ne pouvait que monter. `missing_info` dit au propriétaire ce qui manquait, mais il n'avait aucun moyen de dire « c'est traité », ni d'ailleurs de fournir la réponse manquante autrement qu'en faisant re-crawler une page — ce qui ne sert à rien quand l'information n'est écrite nulle part sur le site.

### Décision
- **Lire l'erreur** (`api/lib/email.js`) : un helper `deliver()` inspecte `{ data, error }` et retourne `{ delivered, error }` à tous les appelants, avec le motif Resend verbatim (`validation_error / 403 / The domain is not verified`). `support_tickets.delivery_error` le conserve.
- **C'est notre incident, pas celui du client** : `alertPlatform()` prévient `ADMIN_EMAIL` quand un email destiné à un client échoue (dédupliqué par motif, fenêtre de 15 min, jamais récursif — une alerte sur une alerte qui échoue serait la même panne deux fois). Les textes côté dashboard ne demandent plus au client d'aller vérifier « son » Resend : il n'a pas de compte Resend. Ils disent ce qui le concerne — un visiteur attend, la demande est conservée, voici comment y répondre — et le `replyTo` du ticket pointe désormais sur le visiteur, donc « Répondre » suffit.
- **Le visiteur n'est plus pénalisé par notre panne** : le ticket étant persisté, un email raté n'est plus « votre demande n'a pas pu être envoyée » mais « votre demande est bien enregistrée » — c'est vrai, elle est dans le dashboard du client. Seul un ticket qu'on n'a pas réussi à *enregistrer* déclenche des excuses.
- **`scripts/tests/test-email-delivery.js`** (branché sur `npm test`) : stubbe `fetch` et vérifie qu'un 403 est rapporté comme non délivré, avec son motif, et qu'il déclenche l'alerte plateforme. C'est la seule protection possible contre un bug dont le chemin heureux et le chemin cassé sont indiscernables.
- **Répondre à une question sans réponse, depuis la conversation** : `missing_info_status` (`'added'` / `'ignored'` / NULL, migration `20260909050000`). Sur un message signalé, le propriétaire écrit directement la réponse dans une zone de texte ; elle est ajoutée à une section **Additional Information** du site (`lib/knowledge-notes.js` → `crawler.update` sur l'URL synthétique `…/#additional-info`, donc chunkée, embeddée et cherchable comme n'importe quelle page) et la question est marquée traitée dans le même geste. « Ignorer » est l'autre issue légitime. Les deux compteurs (dashboard et liste des conversations) ne comptent plus que les questions **non résolues**.
- **Section éditable** : `AdditionalInfoCard` dans les paramètres avancés relit et corrige ces notes. Elles sont exclues de la liste des pages crawlées : `normalizePageUrl` supprime le fragment, donc `…/#additional-info` s'y confondait avec la page d'accueil — et désactiver/réactiver cette ligne aurait re-scanné la vraie page d'accueil *par-dessus* les notes.
- **Correction attenante** : la lecture d'une page pour édition retire maintenant le préfixe `[Source URL: …]` que l'indexeur ajoute à chaque chunk. Il était réinjecté à chaque sauvegarde et s'empilait d'une ligne à chaque édition.
