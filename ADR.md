...
### Contexte
Le besoin de prouver la flexibilit� du syst�me et de fournir un assistant 'Copilot' aux administrateurs de la plateforme.
### D�cision
- Injection du chatbot natif dans l'application React \pps/admin/index.html\ avec une cl� virtuelle \B2B_ADMIN_COPILOT_KEY\.
- Mise en place d'un pont �v�nementiel global (\window.dispatchEvent('b2b_tool_call')\) dans le widget permettant � la fen�tre parente de r�agir aux outils de l'IA sans coupler les deux codebases.
- L'IA peut d�clencher \
avigate_to\ pour changer la vue (dashboard, pricing, leads, about) dans l'application React.
### Cons�quences
- L'utilisateur final (admin) peut utiliser le chatbot pour naviguer dans son propre tableau de bord. Cela d�montre les capacit�s agentiques (Tool Calling -> DOM Action) du produit de fa�on spectaculaire.
