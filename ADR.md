...
- **Demandes de support « perdues »** : `send_support_email` (`api/chat/index.js`) n'existait qu'en tant qu'email — succès ou échec, rien n'était jamais écrit en base. Un échec d'envoi (`RESEND_API_KEY` absente, domaine d'expédition non vérifié chez Resend, rejet par Resend) ne laissait qu'un `console.error` dans un log Edge éphémère, invisible pour le tenant. Impossible de distinguer « le bot n'a pas appelé l'outil » de « l'outil a tourné mais l'email n'est jamais arrivé » — les deux se traduisaient, côté dashboard, par rigoureusement rien.
...
### Décision
- **`support_tickets`** (nouvelle table, migration `20260909040000`, RLS `current_user_owns_tenant` comme `leads`) : chaque appel à `send_support_email` y écrit une ligne — `name`/`email`/`message` du visiteur, `delivered` (le vrai résultat de `sendSupportTicketEmail`), `session_id`, `tenant_id`, `site_id` — que l'envoi Resend réussisse ou non. `delivered = false` sur une ligne existante signifie très précisément « l'outil a tourné, l'email a échoué » (presque toujours un problème de config Resend, pas un bug de code) ; l'absence totale de lignes signifierait que l'outil n'a jamais été appelé.
...
### Conséquences
- Une demande de support qui échoue à l'envoi reste consultable et actionnable dans le dashboard au lieu d'être perdue — et le tenant peut trancher lui-même entre « bug » et « Resend mal configuré » sans avoir à demander.
