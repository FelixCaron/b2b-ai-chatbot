      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h2 className="text-lg font-bold text-dark-900 flex items-center gap-2">
            <LifeBuoy className="w-5 h-5 text-brand-600" /> Support Requests ({filteredTickets.length})
          </h2>
          <p className="text-xs text-gray-500">Every time a visitor asked your assistant for human help — and whether the email actually reached you.</p>
        </div>
          ...
          filteredTickets.map((ticket) => (
            <div key={ticket.id} className="bg-surface-100 border border-dark-900/5 rounded-2xl p-5 flex flex-col h-full shadow-lg">
              <div className="flex items-start justify-between gap-3 mb-3">
                <div>
                  <h3 className="text-sm font-bold text-dark-900">{ticket.name || 'Unknown visitor'}</h3>
              ...
              <div className="mt-auto bg-surface-200 p-3 rounded-xl border border-dark-900/5">
                <h4 className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">Message</h4>
                <p className="text-xs text-gray-600 whitespace-pre-wrap">
                  {ticket.message || <span className="text-gray-600 italic">No message given.</span>}
                </p>
              </div>
