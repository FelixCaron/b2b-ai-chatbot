...
Its own Vercel project (`logafi`), deployed from this directory — the same
arrangement as `apps/widget` (`dorafi-widget`) and `apps/internal-admin`
(`dorafi-staff`), and separate from the product at `dorafi.logafi.com`
(`dorafi-admin`, deployed from the repository root).
...
```bash
node scripts/dev/serve-static.mjs apps/logafi 5174   # from the repo root
```
