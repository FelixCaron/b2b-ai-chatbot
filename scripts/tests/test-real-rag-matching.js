...
if (!VITE_SUPABASE_URL || !SERVICE_ROLE_KEY || !JINA_API_KEY) throw new Error('VITE_SUPABASE_URL, SUPABASE_SECRET_KEY and JINA_API_KEY are required');
...
async function testRealRAG() {
  console.log("==========================================================================");
  console.log("  TEST DE RECHERCHE RAG RÃ‰ELLE SUR DELAFONTAINE.CA (553 CHUNKS INDEXÃ‰S)");
  console.log("==========================================================================\n");
  ...
  console.log(`ðŸ“Œ Domaine: ${bestSite.domain}`);
  console.log(`ðŸ¢ Tenant ID: ${bestSite.tenant_id}`);
  console.log(`ðŸ“¦ Nombre de documents indexÃ©s en base: ${maxChunks} chunks\n`);

  const REAL_USER_QUESTIONS = [
    "Quels types de portes mÃ©talliques fabriquez-vous ?",
    "Avez-vous des cadres en acier isolants ?",
    "Quelle est la rÃ©sistance au feu (fire rating) de vos portes ?",
    "Quel est le dÃ©lai de livraison (leadtime) pour vos commandes ?",
    "OÃ¹ est situÃ©e l'usine principale Ã  Sherbrooke ?",
    "Faites-vous des portes pour les hÃ´pitaux et les Ã©coles ?",
    "Proposez-vous des dÃ©clarations environnementales (EPD / LEED) ?",
    "Avez-vous des portes en acier inoxydable (stainless steel) ?",
    "Qui est le prÃ©sident ou la direction de De La Fontaine ?",
    "Avez-vous des succursales aux Ã‰tats-Unis ?"
  ];
  ...
  for (let i = 0; i < REAL_USER_QUESTIONS.length; i++) {
    ...
    const statusIcon = docs.length > 0 ? 'âœ…' : 'âŒ';
    if (docs.length > 0) totalHits++;

    console.log(`${statusIcon} Q${i + 1}: "${q}"`);
    console.log(`   â”œâ”€ Documents retournÃ©s : ${docs.length} chunk(s) | Temps : ${elapsed}ms | MÃ©thode : ${method}`);

    if (docs.length > 0) {
      const topUrl = docs[0].url || 'Inconnu';
      const snippet = docs[0].content?.substring(0, 140).replace(/\n+/g, ' ');
      console.log(`   â”œâ”€ Top Source URL      : ${topUrl}`);
      console.log(`   â””â”€ ExtraÃ®t de contenu  : "${snippet}..."`);
    } else {
      console.log(`   â””â”€ âš ï¸ AUCUN DOCUMENT RETOURNÃ‰ !`);
    }
    console.log();
  }

  console.log("==========================================================================");
  console.log(`  BASSIN DE TEST RAG COMPLET: ${totalHits} / ${REAL_USER_QUESTIONS.length} questions ont retournÃ© des documents pertinents.`);
  console.log("==========================================================================\n");
}
