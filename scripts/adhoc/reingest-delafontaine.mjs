...
async function ingestPage(url) {
  ...
  if (!jinaRes.ok) { console.log(`âŒ Jina ${jinaRes.status}`); return { success: false, chunks: 0 }; }

  const pageText = await jinaRes.text();
  if (!pageText || pageText.length < 50) { console.log('âŒ Empty'); return { success: false, chunks: 0 }; }
  ...
  const embeddingMode = embeddings ? `âœ¦ ${JINA_API_KEY ? 'Jina AI' : 'mock'}` : 'âš ï¸  mock';
...
async function run() {
  ...
  console.log(`  Embeddings: ${JINA_API_KEY ? `âœ¦ Jina AI (jina-embeddings-v2-base-multilingual)` : 'âš ï¸  Mock (JINA_API_KEY manquante)'}`);
  console.log("â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•\n");
  ...
  if (failCount > 0) console.log(`  âš ï¸  ${failCount} pages failed`);
  console.log("â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•\n");
