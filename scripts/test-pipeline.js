...
async function runTests() {
  ...
  try {
    ...
    console.error("❌ Shared Zod Schemas FAILED:", err);
    failed++;
  }
  ...
  try {
    ...
    console.error("❌ Database Tenant/Site insertion FAILED:", err);
    failed++;
  }
  ...
  try {
    ...
    if (rrfResults.length > 0) {
      ...
      console.error("❌ Hybrid Search returned 0 results");
      failed++;
    }
  } catch (err) {
    console.error("❌ Hybrid Search RPC FAILED:", err);
    failed++;
  }
  ...
  try {
    ...
    console.error("❌ PGMQ Queue Test FAILED:", err);
    failed++;
  }
