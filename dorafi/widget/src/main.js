...
(function () {
  ...
  // ---------------------------------------------------------------------
  // Fail-safe, in that order of priority: this script runs on someone
  // else's website, and the worst thing it could ever do is take that
  // website down with it. It must therefore survive its own backend being
  // slow, broken, or gone for good — a company can close, a domain can
  // lapse, and the snippet stays pasted in a customer's footer for years
  // after either. Three rules follow, and everything below obeys them:
  //
  //   1. Never throw into the host page. The whole build runs inside one
  //      try/catch; anything unexpected removes what we added and leaves
  //      the page exactly as it was.
  //   2. Never block it. The snippet loads async (see buildWidgetSnippet in
  //      the dashboard), we wait for <body> instead of assuming it, and
  //      every network call has a deadline.
  //   3. Never render something that can't work. If the backend can't be
  //      reached at all, the widget removes itself rather than leave a
  //      launcher that opens onto an error.
  // ---------------------------------------------------------------------

  // One widget per page, whatever a site builder does with the snippet
  // (duplicated in a template, injected twice by a tag manager).
  if (window.__dorafiWidgetLoaded) return;
  window.__dorafiWidgetLoaded = true;
  ...
  function start() {
    ...
    try {
      ...
      // Undo anything half-built, then go quiet. A console warning is the
      // most this is ever allowed to cost the site it runs on.
      try { host?.remove(); } catch (e) { /* nothing left to clean up */ }
      try { console.warn("[Dorafi widget] disabled after an unexpected error:", err); } catch (e) {}
    }
    ...
    function buildWidget() {
      ...
      // A deadline, because "unreachable" and "never answers" are the same
      // thing to a visitor and only one of them resolves on its own. 8s is
      // generous for a read of one row; past that we treat the backend as
      // gone (see removeWidget below).
      const initTimeout = typeof AbortController === "function" ? new AbortController() : null;
      const initDeadline = initTimeout ? setTimeout(() => initTimeout.abort(), 8000) : null;

      /** Take the widget off the page for good. Used whenever the assistant
       *  can't serve — no plan, quota spent, or a backend that can't be
       *  reached at all. A site that outlives us shows nothing, never a
       *  launcher that errors on every message. */
      ...
      fetch(`${initEndpoint}?${initParams.toString()}`, {
        ...
        .then((data) => {
          // No usable answer: the API is down, gone, or answering with an
          // error. Whatever the reason, this widget cannot hold a
          // conversation, so it doesn't pretend it can.
          if (!data) {
            removeWidget();
            return;
          }
        ...
        .catch(() => {
          // Network error, CORS failure, an aborted deadline, a domain that
          // no longer resolves — same conclusion as an unusable answer.
          removeWidget();
        })
        .finally(() => {
  ...
  // A snippet in <head>, or an async script that wins the race against the
  // parser, can run before <body> exists — appending to it then throws.
