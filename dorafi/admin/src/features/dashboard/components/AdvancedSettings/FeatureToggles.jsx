...
export default function FeatureToggles({
  ...
  const handleSaveIntegrations = async () => {
    ...
    if (result?.ok === false) {
      setIntegrationsMsg({ text: result.error || t('Could not save — please try again.'), isError: true });
      return;
    }
    setIntegrationsMsg({ text: t('Saved.'), isError: false });
  };
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div className="bg-surface-100 p-5 rounded-xl border border-dark-900/5 flex items-center justify-between gap-4">
        <div>
          <h4 className="text-sm font-bold text-dark-900 flex items-center gap-2 mb-1">
            <RefreshCw className="w-4 h-4 text-brand-600" /> {t('Update Website Knowledge')}
          </h4>
          <p className="text-xs text-gray-500">{t('Re-reads your website from scratch and refreshes what your assistant knows.')}</p>
        </div>
        ...
        >
          ...
          {isCrawling ? t('Rescanning…') : t('Rescan')}
        </button>
      </div>
      ...
      <div className="bg-surface-100 p-5 rounded-xl border border-dark-900/5 flex items-center justify-between relative overflow-hidden">
        {!isPro && (
          <div className="absolute inset-0 bg-dark-950/80 backdrop-blur-sm z-10 flex flex-col items-center justify-center p-4 text-center">
            ...
            <h4 className="text-sm font-bold text-white">{t('Business Feature')}</h4>
            <p className="text-xs text-gray-400 max-w-[250px]">{t('Upgrade to Business to capture visitor emails and contact info as leads.')}</p>
          </div>
        )}

        <div>
          <h4 className="text-sm font-bold text-dark-900 flex items-center gap-2 mb-1">
            <ShieldCheck className="w-4 h-4 text-brand-600" /> {t('Lead Capture & Email Collection')}
          </h4>
          <p className="text-xs text-gray-500">
            {t('Automatically prompts visitors for email and contact info.')}
          </p>
        </div>
        ...
        <button
          ...
          aria-label={t('Toggle lead capture & email collection')}
        >
      ...
      <div className="bg-surface-100 p-5 rounded-xl border border-dark-900/5 flex items-center justify-between">
        <div>
          <h4 className="text-sm font-bold text-dark-900 flex items-center gap-2 mb-1">
            <Settings2 className="w-4 h-4 text-brand-600" /> {t('Widget Accent Color')}
          </h4>
          <p className="text-xs text-gray-500">{t('Match your brand styling.')}</p>
        </div>
      ...
      <div className="bg-surface-100 p-5 rounded-xl border border-dark-900/5 flex items-center justify-between">
        <div>
          <h4 className="text-sm font-bold text-dark-900 flex items-center gap-2 mb-1">
            <Sparkles className="w-4 h-4 text-emerald-600" /> {t('Primary Objective')}
          </h4>
          <p className="text-xs text-gray-500">{t('AI conversation focus.')}</p>
        </div>
        ...
        >
          <option value="support">{t('Information & Support')}</option>
          ...
          <option value="lead" disabled={!isPro}>
            {isPro ? t('Lead Generation & Sales') : t('Lead Generation & Sales (Business)')}
          </option>
        </select>
      </div>
      ...
      <div className="bg-surface-100 p-5 rounded-xl border border-dark-900/5 flex items-center justify-between">
        <div>
          <h4 className="text-sm font-bold text-dark-900 flex items-center gap-2 mb-1">
            <Sparkles className="w-4 h-4 text-emerald-600" /> {t('Voice Tone')}
          </h4>
          <p className="text-xs text-gray-500">{t('Personality & communication style.')}</p>
        </div>
        ...
        <select
          ...
          className="bg-white border border-gray-300 text-dark-900 text-xs rounded-lg px-3 py-2 outline-none"
        >
          <option value="professionnel">{t('Professional & Courteous')}</option>
          <option value="amical">{t('Warm & Friendly')}</option>
        </select>
      </div>
      ...
      <div className="bg-surface-100 p-5 rounded-xl border border-brand-500/20 flex flex-col gap-4 relative overflow-hidden">
        {!isPro && (
          <div className="absolute inset-0 bg-dark-950/80 backdrop-blur-sm z-10 flex flex-col items-center justify-center p-4 text-center">
            <Lock className="w-6 h-6 text-brand-400 mb-2" />
            <h4 className="text-sm font-bold text-white">{t('Business Feature')}</h4>
            <p className="text-xs text-gray-400 mb-3 max-w-[250px]">{t('Upgrade to Business to unlock calendar integrations and support email forwarding.')}</p>
          </div>
        )}

        <div className="flex items-center gap-2 mb-2">
          <Sparkles className="w-5 h-5 text-brand-600" />
          <h4 className="text-base font-bold text-dark-900">{t('Business Integrations')}</h4>
        </div>

        <div className="flex flex-col sm:flex-row gap-4">
          <div className="flex-1">
            <label className="text-xs font-semibold text-gray-500 mb-1 block">{t('Support Email')}</label>
            <input
              type="email"
              placeholder={t('support@yourcompany.com')}
              value={supportEmail}
            ...
            />
            <p className="text-[10px] text-gray-500 mt-1">{t('Where the assistant sends support requests.')}</p>
          </div>

          <div className="flex-1">
            <label className="text-xs font-semibold text-gray-500 mb-1 block">{t('Calendar Link')}</label>
            <input
              type="url"
              placeholder={t('https://calendly.com/your-name')}
              value={calendarLink}
              onChange={(e) => setCalendarLink(e.target.value)}
              className="w-full bg-white border border-gray-300 text-dark-900 text-sm rounded-lg px-4 py-2.5 outline-none focus:border-brand-500/50"
            />
            <p className="text-[10px] text-gray-500 mt-1">{t('Calendly, Cal.com, or Google Calendar link.')}</p>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          >
            ...
            {isSavingIntegrations ? t('Saving...') : t('Save')}
          </button>
        </div>
      </div>
    </div>
