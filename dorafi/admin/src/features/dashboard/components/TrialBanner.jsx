...
export default function TrialBanner({ trialActive, trialExpired, trialDaysLeft, onShowPricing }) {
    ...
    return (
      <div className="bg-red-500/10 border border-red-500/30 rounded-2xl p-4 space-y-3 animate-in fade-in">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 text-xs">
            <h4 className="font-bold text-dark-900 text-sm mb-1">
              {t('Your free trial has ended — the assistant is offline')}
            </h4>
            <p className="text-gray-600 leading-relaxed">
              {t('Nothing was deleted: everything your assistant learned, and every lead it captured, is still here. Choose a plan and it comes straight back online.')}
            </p>
          </div>
        </div>
        <div className="flex justify-end pt-2 border-t border-red-500/20">
          ...
          >
            <Sparkles className="w-3.5 h-3.5" /> {t('Choose a plan →')}
          </button>
        </div>
      </div>
    );
  ...
  const daysLabel =
    trialDaysLeft === null
      ? t('Your Business trial is running')
      : trialDaysLeft === 1
        ? t('1 day left in your Business trial')
        : t('{n} days left in your Business trial', { n: trialDaysLeft });
    ...
    <div className="bg-brand-500/10 border border-brand-500/30 rounded-2xl p-4 space-y-3 animate-in fade-in">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 text-xs">
          ...
          <p className="text-gray-600 leading-relaxed">
            {t('Lead capture, booking redirection and the premium model are all switched on during your trial. Subscribe before it ends to keep them — your assistant stays online without interruption.')}
          </p>
        </div>
      </div>
      <div className="flex justify-end pt-2 border-t border-brand-500/20">
        ...
        >
          <Sparkles className="w-3.5 h-3.5" /> {t('See plans')}
        </button>
      </div>
    </div>
  );
